...
function buildAgentCard(agent) {
  ...
  const imgHtml = hasPhoto
    ? `<img class="agent-photo-img" src="${agent.avatar}" alt="${ename} ${elast}" loading="lazy"
            onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"/>`
    : '';
  const initialsHtml = hasPhoto
    ? `<div class="agent-photo-init" style="background:${bg};display:none">${initials}</div>`
    : `<div class="agent-photo-init" style="background:${bg}">${initials}</div>`;

  const stats = [];
  if (years) stats.push(`<div class="agent-cred"><span class="agent-cred-val">${years}</span><span class="agent-cred-lbl">Años</span></div>`);
  if (license) stats.push(`<div class="agent-cred"><span class="agent-cred-val">Mat.</span><span class="agent-cred-lbl">${license}</span></div>`);
  if (!stats.length) stats.push(`<div class="agent-cred"><span class="agent-cred-val">—</span><span class="agent-cred-lbl">Agente</span></div>`);
  ...
  return `
    <div class="agent-card">
      <div class="agent-photo">
        ${imgHtml}${initialsHtml}
        <div class="agent-vignette"></div>
        <div class="agent-photo-overlay">
          <h3 class="agent-name">${ename} ${elast}</h3>
          ${espec ? `<div class="agent-role">${espec}</div>` : ''}
        </div>
      </div>
      <div class="agent-body">
        <div class="agent-creds">${stats.join('')}</div>
        ${ebio ? `<p class="agent-bio">${ebio}</p>` : ''}
        <div class="agent-foot">${license ? 'Mat. CPI N° ' + license : 'Asesor inmobiliario'}${years ? ' · ' + years + ' años de experiencia' : ''}</div>
